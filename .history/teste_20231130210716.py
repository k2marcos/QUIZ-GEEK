...
def AbrirResultados():
    ...
    if pontos == 100:
            msg='EXCELENTE!\nVAI VIVER SUA VIDA'
    elif pontos <= 95 and pontos > 75:
            msg='VOCÊ FOI BEM \n MAS NÃO É O MÁXIMO'
    elif pontos <= 75 and pontos > 50:
            msg='COMEÇOU A VÊ ANIME AGORA?'
    else:
            msg='CHISPA, NERD FAKE\nVAZA DAQUI!'
    
    Lb_bemvindo= Label(root, text="Pontuação = "+str(pontos)+"\n"+msg, background="#FFF", foreground="#000000", width=19, height=6)
    Lb_bemvindo.place(x=155, y=352)
    Lb_bemvindo.configure(font=("negrito", 22, "italic"))
