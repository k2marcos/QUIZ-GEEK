...
def AbrirResultados():
    ...
    Lb_bemvindo= Label(root, text="Pontuação = "+str(pontos), background="#ffffff", foreground="#000000", width=15, height=5)
    if  pontos== 200:
        print('EXCELENTE! Você é NERD DE CARTEIRINHA')
    elif pontos <= 190 and pontos > 150:
        print('VOCÊ FOI BEM, MAS NÃO É O MÁXIMO')
    elif pontos <= 150 and pontos > 100:
        print('Assista mais animes')
    else:
        print('CHISPA, NERD FAKE')
        print('VAZA DAQUI!')
    Lb_bemvindo.place(x=170, y=360)
    Lb_bemvindo.configure(font=("negrito", 25, "italic"))


    btn_play = Button(root,text="REPLAY",compound=CENTER,width=8, height=2,border=False,bg="#FAEA0C",command=lambda:comecar())
    btn_play.place(x=236, y=780)
    btn_play.configure(font=("negrito", 30, "italic"))
